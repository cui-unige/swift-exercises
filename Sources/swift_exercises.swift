...
struct Pokemon {
    let nickname          : String?
    let hitpoints         : Int // remaining hitpoints equal to zero at the beginning
    let size              : Int
    let weight            : Int
    let experience        : Int
    let level             : Int
...
var effective_stats : Stats {
 // effective_stats for Generations I and II)
get {
      let s

let hitpoints: Int = ((((self.species.base_values.hitpoints + self.individual_values.hitpoints)*2 + (self.effort_values.hitpoints.squareRoot().rounded(.up)/4).rounded(.down))*level)/100).round(.down) + level + 10
let attack: Int = ((((self.species.base_values.attack + self.individual_values.attack)*2 + (self.effort_values.attack.squareRoot().rounded(.up)/4).rounded(.down))*level)/100).round(.down) + 5
let defense: Int = ((((self.species.base_values.defense + self.individual_values.defense)*2 + (self.effort_values.defense.squareRoot().rounded(.up)/4).rounded(.down))*level)/100).round(.down) + 5
let special_attack: Int = ((((self.species.base_values.special_attack + self.individual_values.special_attack)*2 + (self.effort_values.special_attack.squareRoot().rounded(.up)/4).rounded(.down))*level)/100).round(.down) + 5
let special_defense: Int = ((((self.species.base_values.special_defense + self.individual_values.special_defense)*2 + (self.effort_values.special_defense.squareRoot().rounded(.up)/4).rounded(.down))*level)/100).round(.down) + 5
let speed: Int = ((((self.species.base_values.speed + self.individual_values.speed)*2 + (self.effort_values.speed.squareRoot().rounded(.up)/4).rounded(.down))*level)/100).round(.down) + 5
return Stats        }}}
