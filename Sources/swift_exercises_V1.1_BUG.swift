...
let speciesEmpty : Set<Species> = []
let Attaque_lugia = [aeroblast, earthquake, recover, curse]
let Values_lugia = Stats(hitpoints: 106,attack: 90,defense: 130,special_attack: 90,special_defense: 154,speed: 110)
let individual_Values_lugia = Stats(hitpoints: 106,attack: 90,defense: 130,special_attack: 90,special_defense: 154,speed: 110)
let lugia_EffortValues = Stats(
    hitpoints: 0,
...
let allAttacksEntei = [FireBlast, SolarBeam, SunnyDay, ReturnRest]
let entei_Specie = Species(id: 244, name: "Entei", evolutions: [], attacks: allAttacksEntei, type: (.fire, nil), base_values: enteiBaseValues)
let entei = Pokemon(nickname: "Martin", hitpoints: 120, size: 5, weight: 230, experience: 0, level: 1, nature: Nature.naughty, species: entei_Specie, moves: allAttacksEntei, individual_values: enteiIndividualValues, effort_values: enteiEffortValues)
...
//pokemon 4
let FireBlast2 = Move(id: 1, name: "Fire BLast2", description: "30% chance to burn the target.", category: Category.special, type: Type.fire, power: 120, accuracy: 85, powerpoints: 5, priority: 0)
let BodySlam2 = Move(id: 2, name: "Roar2", description: "30% chance to paralyze the target", category: Category.physical, type: Type.normal, power: 85, accuracy: 100, powerpoints: 15, priority: 0)
let HyperBeam2 = Move(id: 3, name: "Hyper Beam2", description: "User cannot move next turn, unless opponent or Substitute was KOed.", category: Category.physical, type: Type.normal, power: 150, accuracy: 90, powerpoints: 5, priority: 0)
let Reflect2 = Move(id: 4, name: "Reflect2", description: "Lowers the physical damage done to user.", category: Category.status, type: Type.psychic, power: 0, accuracy: 0, powerpoints: 20, priority: 0)
let stats_arca2 = Stats(hitpoints: 90, attack: 110, defense: 80, special_attack: 80, special_defense: 80, speed: 95)
let stats_arca_indiv2 = Stats(hitpoints: 90, attack: 110, defense: 80, special_attack: 80, special_defense: 80, speed: 95)

let arca_EffortValues2 = Stats(
    hitpoints: 0,
    attack: 2,
    defense: 0,
    special_attack: 0,
...
let attacks_arca2 = [FireBlast2, BodySlam2, HyperBeam2, Reflect2]
let arcanine_spec2 = Species(id: 60, name: "arcanine_2", evolutions: [], attacks: attacks_arca2, type: (Type.fire, nil), base_values: stats_arca2)

let arcanine2 = Pokemon(nickname: "pepito", hitpoints: 60, size: 3, weight: 155, experience: 0, level: 1, nature: Nature.rash, species: arcanine_spec2, moves: attacks_arca2, individual_values: stats_arca_indiv2, effort_values: arca_EffortValues2)
...
func battle(trainers: inout [Trainer]) -> ()
{
    ...
    if currentState.pokemon_attack.effective_stats.speed > currentState.pokemon_defense.effective_stats.speed && battle_part2 == 1
    {
      ...
      var choice_battle: Int = 0
        ...
        else
        {
          ...
          {
            currentState.pokemon_attack.moves[0].powerpoints -= 1 //Decreasing PP
            martin_user.pokemons[currentPokemon_J1].moves[choice_battle].powerpoints -= 1
            if (currentState.pokemon_attack.moves[0].powerpoints == 0)
            {
              print("\nNo PP for the move")
              currentState.pokemon_attack.moves.remove(at: choice_battle)
              print("bybybb") //Removing move
              //martin_user.pokemons[currentPokemon_J1].moves.remove(at: choice_battle)
              print("fenununuuuuu")//Updating bob's pokemon
            }
            else
            {
              currentState.pokemon_attack.moves[choice_battle] = currentState.pokemon_attack.moves[0] //Updating global attacks list
            }
            if ((currentState.pokemon_attack.nickname == "Martin") && (currentState.pokemon_attack.moves[0].id == 3))
            {
              currentState.pokemon_environment.weather = Weather.clear_skies //Updating conditions
            }
          }
        }
        damages = damage(environment: currentState.pokemon_environment, pokemon: currentState.pokemon_attack, move: currentState.pokemon_attack.moves[0], target: currentState.pokemon_defense)
        currentState.pokemon_defense.hitpoints = currentState.pokemon_defense.hitpoints - damages; //Remaining hitpoints

        if currentState.pokemon_defense.hitpoints <= 0 {

            print("Pokemon ennemi was killed !\n");
            print("Selected move: \(currentState.pokemon_defense.moves[0].name). Damages done: \(damages) \n");
            pepito_user.pokemons.remove(at: currentPokemon_J2)
            if nb_currenPokemon_J2 > 0
            {


              currentState.pokemon_defense = pepito_user.pokemons[0]
              currentState.pokemon_defense.moves[0] = currentState.pokemon_defense.moves[0]
              nb_currenPokemon_J2 = nb_currenPokemon_J2 - 1
            }
            else
            {
              abort();
        ...
        print("powerpoints defense \(currentState.pokemon_defense.moves[0].powerpoints)")
        print("powerpoints attack \(currentState.pokemon_attack.moves[0].powerpoints)")
    }
    else
    {
      print("powerpoints defense \(currentState.pokemon_defense.moves[0].powerpoints)")
      print("powerpoints attack \(currentState.pokemon_attack.moves[0].powerpoints)")
      damages = damage(environment: currentState.pokemon_environment, pokemon: currentState.pokemon_defense, move: currentState.pokemon_defense.moves[0], target: currentState.pokemon_attack)
      currentState.pokemon_attack.hitpoints = currentState.pokemon_attack.hitpoints - damages;

      currentState.pokemon_defense.moves[0].powerpoints -= 1 //Decreasing PP

      if (currentState.pokemon_defense.moves[0].powerpoints == 0){
          print("No PP for the move")
          currentState.pokemon_defense.moves.remove(at:  0) //Removing move
      }
      ...
      if currentState.pokemon_attack.hitpoints <= 0 {
          ...
          print("Pokemon was killed !\n")
          //
          martin_user.pokemons.remove(at: currentPokemon_J1)
          if nb_currenPokemin_J1 > 0
          {
            currentState.pokemon_attack =  martin_user.pokemons[0]
            currentState.currentState.pokemon_attack.moves[0] = martin_user.pokemons[0].moves[0]
            nb_currenPokemin_J1 = nb_currenPokemin_J1 - 1
          }
          else
          {
            abort();
          ...
          */
          ...
          print("Selected move: \(currentState.pokemon_defense.moves[0].name). Damages done: \(damages) \n");
          martin_user.pokemons.remove(at: currentPokemon_J1)
          if nb_currenPokemon_J1 > 0
          {


            currentState.pokemon_attack = martin_user.pokemons[0]
            currentState.pokemon_attack.moves[0] = martin_user.pokemons[0].moves[0]
            nb_currenPokemon_J1 = nb_currenPokemon_J1 - 1
          }
          else
          {
            abort();
    ...
    if currentState.pokemon_attack.effective_stats.speed > currentState.pokemon_defense.effective_stats.speed && battle_part2 == 1
    {
        print("Pokemon Attack : \(currentState.pokemon_attack.nickname)\t| Specie: \(currentState.pokemon_attack.species.name)");
        print("Pokemon Defense : \(currentState.pokemon_defense.nickname)\t| Specie: \(currentState.pokemon_defense.species.name)\n");
        print("Selected move : \(currentState.pokemon_attack.moves[0].name). Damages done: \(damages) \n");
        battle_part2 = 0 //Changing player
    }
    else
    {
...
var martin_user = Trainer(pokemons: [lugia, entei])
var tour = 2
var pepito_user = Trainer(pokemons: [arcanine, arcanine2])
var trainers = [martin_user, pepito_user]
var currentPokemon_J1 = 0
var nb_currenPokemon_J1 = 2
var nb_currenPokemon_J2 = 2
var currentPokemon_J2 = 0
var damages: Int = 0
let Game_environnement = Environment(weather: Weather.sandstorm, terrain: Terrain.grassy)
      ...
      print("\nSur linux on ne peut pas utilisé readLine donc on prend toujours l'autre\n")
         ...
         var move_player_2 = Int(arc4random_uniform(3))
              ...
              if (move_player_2 > martin_user.pokemons.count){
                  ...
                  currentPokemon_J1 = (currentPokemon_J1 + 1) % 2
                  currentState.pokemon_attack = martin_user.pokemons[currentPokemon_J1]
              }
